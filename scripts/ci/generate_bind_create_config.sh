...
json_payload1=$(echo "{\"username\":\"$SMB_USERNAME\",\"password\":\"$SMB_PASSWORD\"}" | sed 's/"/\\"/g')
json_payload2=$(echo "{\"username\":\"$SMB_USERNAME\",\"password\":\"$SMB_PASSWORD\", \"mount\": \"/var/vcap/data/foo\"}" | sed 's/"/\\"/g')
